...
(function (factory) {
  ...
  var Comments = {
    ...
    validateAttachment: function (files) {
      ...
      this.$el
        .find(".commenting-field")
        .find(".textarea-wrapper")
        .find(".msgContainer")
        .remove();
      if (
        this.options.attachmentFileFormats
          .toLowerCase()
          .indexOf(fileExtn.toLowerCase()) < 0 &&
        this.options.attachmentFileFormats
          .toLowerCase()
          .indexOf(fileType.toLowerCase()) < 0 &&
        this.options.attachmentFileFormats
          .toLowerCase()
          .indexOf(fileType1.toLowerCase()) < 0
      ) {
        var message = this.createMessage(
          `Allowed File formats: ${this.options.attachmentFileFormats}`,
          "error"
        );
        this.$el
          .find(".commenting-field")
        ...
        if (fileSize > this.options.attachmentFileSize) {
          var message = this.createMessage(
            `Allowed File Size: ${this.options.attachmentFileSize} MB`,
            "error"
          );
          this.$el
            .find(".commenting-field")
      ...
      if (fileCount) {
        if (this.validateAttachment(files)) {
          ...
          var success = function (commentArray) {
            ...
            if (
              commentArray.length == fileCount &&
              self.getTextareaContent(textarea).length == 0
            ) {
    ...
    replyButtonClicked: function (ev) {
      ...
      if (previousParentId != parentId) {
        ...
        var endOfReply =
          scrollTop + replyField.position().top + replyField.outerHeight();
        var endOfScrollable =
          scrollTop + this.options.scrollContainer.outerHeight();
    ...
    createCommentingFieldElement: function (
      ...
      if (existingCommentId) {
        ...
        if (this.isAllowedToDelete(existingCommentId))
          deleteButton.addClass("enabled displayctrl");
    ...
    createCommentWrapperElement: function (commentModel) {
      ...
      if (isAttachment) {
        ...
        if (this.options.enableDocumentPreview) {
          ...
          // } else
          if (
            commentModel.fileID !== undefined &&
            commentModel.fileID !== "" &&
            (format.toLowerCase().indexOf("excel") >= 0 ||
              format.toLowerCase().indexOf("word") >= 0 ||
              format.toLowerCase().indexOf("presentation") >= 0)
          ) {
            var docPreview = $("<iframe/>", {
          ...
          // Case: image preview
            ...
            var video = $("<iframe/>", {
              src: `${
                this.options.origin + this.options.siteURL
              }/_layouts/15/embed.aspx?UniqueId=${
                commentModel.fileID
              }&client_id=FileViewerWebPart&embed={"af":false,"id":"${
                commentModel.fileID
              }","o":"${this.options.origin}","p":1,"z":"width"}`,
              type: commentModel.fileMimeType,
              allow: "autoplay; encrypted-media",
              width: "300",
              height: "300",
            ...
            if (format.toLowerCase().indexOf("excel") >= 0)
              ...
              format.toLowerCase().indexOf("zip") >= 0 ||
              format.toLowerCase().indexOf("rar") >= 0
            )
      ...
      if (
        ...
        if (
          isAttachment &&
          this.isAllowedToDelete(commentModel.id) &&
          ...
          console.log(this.isAllowedToDelete(commentModel.id) )
          if (this.isAllowedToDelete(commentModel.id) && this.options.enableDeleting) {
            var deleteButton = $('<button/>', {
                'class': 'action delete enabled',
                text: this.options.textFormatter(this.options.deleteText)
            });
  ...
  $(document).on('click', 'jquery-comments .likes', function(){
    $(this).toggle('active');
  })
})
